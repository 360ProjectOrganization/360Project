...
// GET: api/job-postings/comments/me 
router.get('/comments/me', requireAuth, async (req, res) => {
  try {
    const comments = await jobPostingService.findCommentsByUserId(req.user.id, req.user.role);
    res.json(comments);
  }
  catch (err) {
